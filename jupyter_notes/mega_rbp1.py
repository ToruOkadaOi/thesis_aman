...
peakachu_file = "peakachu_merged_loops_rbp1.bedpe"
mustache_file = "mustache_merged_loops_rbp1_fixed_clean.bedpe"
cooldots_file = "cooldots_merged_loops_rbp1.bedpe"
...
extract_original_bedpe(df_peakachu, only_p, "unique_peakachu_rbp1_fixed.bedpe")
extract_original_bedpe(df_mustache, only_m, "unique_mustache_rbp1_fixed.bedpe")
extract_original_bedpe(df_cooldots, only_c, "unique_cooldots_rbp1_fixed.bedpe")
...
# Add total loop counts as annotations near each circle
...
plt.text(0.0, -0.9, f"Cooldots total: {len(cooldots_ids)}", fontsize=10, color='black', ha='center')

# Finalize plot
plt.title(f"EED mutant Loop Overlap (±{max_dist // 1000}kb Midpoint)")
plt.savefig("mergedloops_eed_mut_fixed.png", dpi=300, bbox_inches='tight')
plt.show()

# ------------------------------
# Create merged mega loop list
# ------------------------------

mega_ids = only_p | only_m | only_c | p_and_m_only | p_and_c_only | m_and_c_only | p_in_all
all_loops_df = pd.concat([df_peakachu, df_mustache, df_cooldots]).drop_duplicates(subset='loop_id')
extract_original_bedpe(all_loops_df, mega_ids, "mega_merged_loops_rbp1.bedpe")